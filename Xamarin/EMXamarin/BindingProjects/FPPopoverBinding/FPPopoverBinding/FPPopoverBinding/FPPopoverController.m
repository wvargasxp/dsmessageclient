...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint
{
    ...
    if(windows.count > 0)
    {
        ...
        // added by james: for our specific case, the view we're looking for is at 1 not 0
        //NSLog(@"if _window.subviews.count > 0");
        if(_window.subviews.count > 0)
        {
            //NSLog(@"_parentView = = [_window.subviews objectAtIndex:0]");
            _parentView = [_window.subviews objectAtIndex:0];

            if (!_parentView) {
                //NSLog(@"[windows objectAtIndex:1]");
                if ([windows objectAtIndex:1]) {
                    //NSLog(@"_window = [windows objectAtIndex:1];");
           ...
           // NSLog(@"[windows objectAtIndex:1]");
            if ([windows objectAtIndex:1]) {
                //NSLog(@"else: _window = [windows objectAtIndex:1];");
                _window = [windows objectAtIndex:1];
                //NSLog(@"else: if (_window.subviews.count > 0");
                if (_window.subviews.count > 0) {
                    //NSLog(@"else: _parentView = [_window.subviews objectAtIndex:0]");
                    _parentView = [_window.subviews objectAtIndex:0];
                }
            }

        }
        //NSLog(@"_parentView addSubview:self.view");
        [_parentView addSubview:self.view];
        //NSLog(@"_viewController viewDidAppear:YES");
